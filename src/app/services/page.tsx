"use client";

import { useEffect, useState } from "react";
import Image from "next/image";
import useEmblaCarousel from "embla-carousel-react";
import { services3hr } from "@/app/data/services3hr";
import { importantNotes } from "@/app/data/importantNotes";

type TabType = "3hr" | "4hr";



export default function CleaningServices() {
  const [activeTab, setActiveTab] = useState<TabType>("3hr");
const [activeTab2, setActiveTab2] = useState<TabType>("3hr");

  // Carousel 1 (3hr services)
  const [emblaRef, emblaApi] = useEmblaCarousel({
    align: "start",
    loop: true,
    dragFree: false,
  });

  // Carousel 2 (second service section)
  const [emblaRef2, emblaApi2] = useEmblaCarousel({
    align: "start",
    loop: true,
    dragFree: false,
  });

  const scrollPrev = () => emblaApi && emblaApi.scrollPrev();
  const scrollNext = () => emblaApi && emblaApi.scrollNext();

  const scrollPrev2 = () => emblaApi2 && emblaApi2.scrollPrev();
  const scrollNext2 = () => emblaApi2 && emblaApi2.scrollNext();

  // Auto-loop the carousel
  useEffect(() => {
    if (!emblaApi) return;

    const intervalId = setInterval(() => {
      if (!emblaApi) return;
      emblaApi.scrollNext();
    }, 4000); // change slide every 4 seconds

    return () => clearInterval(intervalId);
  }, [emblaApi]);

  useEffect(() => {
    if (!emblaApi2) return;
  
    const intervalId = setInterval(() => {
      if (!emblaApi2) return;
      emblaApi2.scrollNext();
    }, 4000);
  
    return () => clearInterval(intervalId);
  }, [emblaApi2]);

  return (
    <div className="w-full bg-gray-100 py-5 lg:py-10">
      {/* SERVICE TYPE 1 */}
      <div className="w-[96%] mx-auto px-3 sm:px-4 lg:px-6 rounded-none lg:rounded-xl flex flex-col gap-5 mt-10">

        {/* TOP: IMAGE + CONTENT */}
        <div className="flex flex-col lg:flex-row gap-8 items-stretch">
          {/* LEFT IMAGE */}
          <div className="relative w-full lg:w-[35%] aspect-[4/5] lg:aspect-auto lg:min-h-full flex-shrink-0">
          <Image
            src="/assets/da-12.jpg"
            alt="Cleaning"
            fill
            className="object-cover rounded-xl"
          />
        </div>

          {/* RIGHT CONTENT */}
          <div className="flex-1">

        {/* HEADER */}
        <div className="mb-4">
          <h2 className="text-2xl md:text-3xl font-semibold text-gray-900">
          3hr / 4hr Cleaning
          </h2>
          <p className="text-sm text-gray-600 mt-1">
          Every home is unique oasis. Get a custom cleaning experience with a dedicated professional! Choose between a 3-hour or 4-hour service and let us take care of your cleaning needs!
          </p>
        </div>

        {/* TABS */}
        <div className="inline-flex bg-[#d9dbc9] rounded-lg p-1 mb-6">
          {["3hr", "4hr"].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab as TabType)}
              className={`px-5 py-2 rounded-md font-medium transition-all duration-300 ${
                activeTab === tab
                  ? "bg-white shadow-sm"
                  : "text-gray-600 hover:text-black"
              }`}
            >
              {tab} Cleaning
            </button>
          ))}
        </div>

        {/* SERVICES CAROUSEL */}
        {activeTab === "3hr" && (
          <div className="relative mb-8">

            {/* ARROWS */}
            <button
              onClick={scrollPrev}
              className="absolute -left-4 top-1/2 -translate-y-1/2 bg-white shadow-md rounded-full w-9 h-9 flex items-center justify-center z-10"
            >
              ←
            </button>

            <button
              onClick={scrollNext}
              className="absolute -right-4 top-1/2 -translate-y-1/2 bg-white shadow-md rounded-full w-9 h-9 flex items-center justify-center z-10"
            >
              →
            </button>

            {/* EMBLA VIEWPORT */}
            <div className="overflow-hidden" ref={emblaRef}>
              <div className="-mx-3 flex">
                {services3hr.map((service) => (
                  <div
                    key={service.title}
                    className="flex-[0_0_50%] px-3"
                  >
                    <div className="h-full bg-white border rounded-lg p-6">
                      <h3 className="font-bold mb-3">{service.title}</h3>

                      <ul className="list-disc pl-5 text-sm space-y-2">
                        {service.items.map((item, i) => (
                          <li key={i}>{item}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
                ))}

              </div>
            </div>
          </div>
        )}

        {/* 4HR CONTENT */}
        {activeTab === "4hr" && (
          <div className="grid md:grid-cols-2 gap-6 mb-6">

            <div className="bg-white border rounded-lg p-6">
              <h3 className="font-bold mb-4">SCOPE</h3>

              <p className="text-sm text-gray-600 leading-relaxed">
                The 4-hours cleaning service includes everything in our
                3-hour services as well as cleaning of window frames and
                sill, perfect for 4-5 room apartments or if you want more
                thorough cleaning for your 2-3 room apartment.
              </p>
            </div>

            <div className="bg-white border rounded-lg p-6">
              <h3 className="font-bold mb-4">ADDITIONAL DETAILS</h3>

              <p className="text-sm text-gray-600">
                Includes deeper cleaning areas and window frames that are
                not covered in the standard 3-hour service.
              </p>
            </div>

          </div>
        )}

            {/* IMPORTANT NOTES */}
            <div className="bg-white border rounded-lg overflow-hidden">
              <div className="w-full flex justify-between items-center p-5 font-semibold">
                IMPORTANT NOTES
              </div>

              <div className="p-6 pt-0">
                <ul className="list-disc pl-5 text-sm space-y-2 mb-4">
                <li>
                  Please be reminded that cleaning supplies & equipment are to
                  be provided by you
                </li>
                <li>
                  Additional services as listed below are <b>NOT</b> included
                </li>
              </ul>

              <ol className="list-decimal pl-5 text-sm space-y-2">
                {importantNotes.map((note, i) => (
                  <li key={i}>{note}</li>
                ))}
              </ol>
              </div>
            </div>
          </div>
        </div>

        {/* RATING & PRICING SECTION */}
        <div className="w-full bg-gradient-to-r from-[#fbe9e4] to-[#fdf5f1] py-10 rounded-xl">
          <div className="flex flex-col md:flex-row items-stretch gap-10 px-4 lg:px-8">
            {/* LEFT: RATING & COPY */}
            <div className="flex-1 flex flex-col justify-center">
          <div className="mb-4">
            <p className="text-4xl font-semibold text-gray-900">4.9</p>
            <p className="text-yellow-500 text-xl mt-1">★★★★★</p>
          </div>
          <div className="space-y-4 text-sm text-gray-700 max-w-md">
            <div>
              <p className="font-semibold tracking-wide">TRUSTED PROFESSIONALS</p>
              <p>Our crew are all employed and certified in accordance to the law.</p>
            </div>
            <div>
              <p className="font-semibold tracking-wide">DEDICATED SUPPORT</p>
              <p>Our support team will assist on all your needs, from booking to post service support. (Mon–Sun, from 9am–6pm)</p>
            </div>
          </div>
        </div>

            {/* RIGHT: PRICING TABLE */}
            <div className="flex-1">
          <div className="bg-white rounded-lg shadow-sm overflow-hidden h-full">
            <div className="bg-[#d7b8a0] text-white text-center py-3 font-semibold tracking-wide">
              PRICING
            </div>
            <div className="grid grid-cols-2 text-xs sm:text-sm font-semibold text-gray-700 border-b">
              <div className="py-3 pl-5 border-r flex items-center gap-2">
                <span className="text-lg">⏱</span>
                <span>HRS</span>
              </div>
              <div className="py-3 pl-5 flex items-center gap-2">
                <span className="text-lg">💲</span>
                <span>PRICE</span>
              </div>
            </div>
            <div className="divide-y text-xs sm:text-sm">
              <div className="grid grid-cols-2">
                <div className="py-4 pl-5 border-r flex items-center">
                  <span>3hr Clean</span>
                </div>
                <div className="py-4 pl-5">
                  <p className="font-semibold">$75</p>
                  <p className="text-gray-600 text-[11px] sm:text-xs">9am – 12pm / 2pm – 5pm</p>
                </div>
              </div>
              <div className="grid grid-cols-2">
                <div className="py-4 pl-5 border-r flex items-center">
                  <span>4hr Clean</span>
                </div>
                <div className="py-4 pl-5">
                  <p className="font-semibold">$100</p>
                  <p className="text-gray-600 text-[11px] sm:text-xs">8am – 12pm / 2pm – 6pm</p>
                </div>
              </div>
            </div>
          </div>
            </div>
          </div>
        </div>
      </div>
      

      

{/* SERVICE TYPE 2 */}
<div className="w-[96%] mx-auto px-3 sm:px-4 lg:px-6 rounded-none lg:rounded-xl flex flex-col gap-5 mt-10">
{/* TOP: IMAGE + CONTENT */}
<div className="flex flex-col lg:flex-row gap-8 items-stretch">
  {/* LEFT IMAGE */}
  <div className="relative w-full lg:w-[35%] aspect-[4/5] lg:aspect-auto lg:min-h-full flex-shrink-0">
  <Image
    src="/assets/da-11.jpg"
    alt="Cleaning"
    fill
    className="object-cover rounded-xl"
  />
</div>

  {/* RIGHT CONTENT */}
  <div className="flex-1">

     {/* HEADER */}
     <div className="mb-4">
          <h2 className="text-2xl md:text-3xl font-semibold text-gray-900">
          Evening cleaning
          </h2>
          <p className="text-sm text-gray-600 mt-1">
          Ditch the cleaning after work! This 3-hour evening service lets you come home to a spotless haven, perfect for unwinding and recharging. <br/>Don’t let housework eat into your precious free time. Our evening cleaning service, available from 7pm to 10pm, is designed for busy working professionals and working parents. Relax and let us take care of the cleaning after a long day, and focus on what truly matters.         </p>
        </div>



{/* SERVICES CAROUSEL */}
{activeTab === "3hr" && (
  <div className="relative mb-8">

    {/* ARROWS */}
    <button
      onClick={scrollPrev2}
      className="absolute -left-4 top-1/2 -translate-y-1/2 bg-white shadow-md rounded-full w-9 h-9 flex items-center justify-center z-10"
    >
      ←
    </button>

    <button
      onClick={scrollNext2}
      className="absolute -right-4 top-1/2 -translate-y-1/2 bg-white shadow-md rounded-full w-9 h-9 flex items-center justify-center z-10"
    >
      →
    </button>

    {/* EMBLA VIEWPORT */}
    <div className="overflow-hidden" ref={emblaRef2}>
      <div className="-mx-3 flex">
        {services3hr.map((service) => (
          <div
            key={service.title}
            className="flex-[0_0_50%] px-3"
          >
            <div className="h-full bg-white border rounded-lg p-6">
              <h3 className="font-bold mb-3">{service.title}</h3>

              <ul className="list-disc pl-5 text-sm space-y-2">
                {service.items.map((item, i) => (
                  <li key={i}>{item}</li>
                ))}
              </ul>
            </div>
          </div>
        ))}

      </div>
    </div>
  </div>
)}



    {/* IMPORTANT NOTES */}
    <div className="bg-white border rounded-lg overflow-hidden">
      <div className="w-full flex justify-between items-center p-5 font-semibold">
        IMPORTANT NOTES
      </div>

      <div className="p-6 pt-0">
        <ul className="list-disc pl-5 text-sm space-y-2 mb-4">
        <li>
          Please be reminded that cleaning supplies & equipment are to
          be provided by you
        </li>
        <li>
          Additional services as listed below are <b>NOT</b> included
        </li>
      </ul>

      <ol className="list-decimal pl-5 text-sm space-y-2">
        {importantNotes.map((note, i) => (
          <li key={i}>{note}</li>
        ))}
      </ol>
      </div>
    </div>
  </div>
</div>

{/* RATING & PRICING SECTION */}
<div className="w-full bg-gradient-to-r from-[#fbe9e4] to-[#fdf5f1] py-10 rounded-xl">
  <div className="flex flex-col md:flex-row items-stretch gap-10 px-4 lg:px-8">
    {/* LEFT: RATING & COPY */}
    <div className="flex-1 flex flex-col justify-center">
  <div className="mb-4">
    <p className="text-4xl font-semibold text-gray-900">4.9</p>
    <p className="text-yellow-500 text-xl mt-1">★★★★★</p>
  </div>
  <div className="space-y-4 text-sm text-gray-700 max-w-md">
    <div>
      <p className="font-semibold tracking-wide">TRUSTED PROFESSIONALS</p>
      <p>Our crew are all employed and certified in accordance to the law.</p>
    </div>
    <div>
      <p className="font-semibold tracking-wide">DEDICATED SUPPORT</p>
      <p>Our support team will assist on all your needs, from booking to post service support. (Mon–Sun, from 9am–6pm)</p>
    </div>
  </div>
</div>

    {/* RIGHT: PRICING TABLE */}
    <div className="flex-1">
  <div className="bg-white rounded-lg shadow-sm overflow-hidden h-full">
    <div className="bg-[#d7b8a0] text-white text-center py-3 font-semibold tracking-wide">
      PRICING
    </div>
    <div className="grid grid-cols-2 text-xs sm:text-sm font-semibold text-gray-700 border-b">
      <div className="py-3 pl-5 border-r flex items-center gap-2">
        <span className="text-lg">⏱</span>
        <span>HRS</span>
      </div>
      <div className="py-3 pl-5 flex items-center gap-2">
        <span className="text-lg">💲</span>
        <span>PRICE</span>
      </div>
    </div>
    <div className="divide-y text-xs sm:text-sm">
      <div className="grid grid-cols-2">
        <div className="py-4 pl-5 border-r flex items-center">
          <span>3hr Evening Clean</span>
        </div>
        <div className="py-4 pl-5">
          <p className="font-semibold">$70</p>
        </div>
      </div>
      
    </div>
  </div>
    </div>
  </div>
</div>
</div>


    </div>
  );
}